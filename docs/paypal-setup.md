...
The launcher additionally requires `.private-showcase/neon-preview.env`, verifies
its database endpoint differs from `DATABASE_URL_MAINLIVE`, and selects Preview
database routing even under local `NODE_ENV=production`. No Live PayPal secrets
are loaded. `scripts/with-preview-database.mjs` applies the same guard for local
builds, Prisma commands and isolated test tooling.
The custom-credit migrations add the nullable cart amount and replace the old
68 NOK per-order database check with 391.70 NOK, the maximum supported mixed
cart. The serialized daily cap remains 500 NOK. Migration history is append-only;
both changes are applied to isolated Preview only until production promotion.
...
the user's fiat/crypto display preference.
