...
16. **Company org-number verification is email-confirmed.** On `/companies/create`, org numbers are looked up via Brønnøysund and stored as `CompanyOrgVerification` PENDING. `Company.orgNumber`/`orgType` are only linked after official-email confirmation (`/api/companies/verify-org`). If unconfirmed/denied, company remains unverified with no legal org link.
17. **Company product creation is warehouse-gated.** Any company-context flow into `/products/create` (hub deep-link or manual company selection) enters digital-only lite mode when the selected company has no warehouse address; backend blocks PHYSICAL/HYBRID creation until a warehouse exists.
16. **AI generation requires authentication + auto-resolves keys.** Server uses `auto` mode: saved key → OpenAI platform key for owner (`PLATFORM_OWNER_EMAIL`) or paid-entitled buyers (daily-cap products: `AI_PAID_PRODUCT_IDS`, or finite credit packs: `AI_PAID_CREDIT_PACK_PRODUCTS`) → Groq free tier (GROQ_API_KEY, Llama 3.3 70B) → error. Platform-mode quota is 5/day by default; paid users can be governed by daily caps (`AI_PAID_DEFAULT_DAILY_LIMIT` / `AI_PAID_PRODUCT_DAILY_LIMITS`) or finite poll-credit packs (recommended for unit economics). BYOK unlimited — supports 6 providers: **OpenAI** (GPT-5.2/5.1/4.1 family, o4-mini), **Anthropic** (Claude Sonnet 4.6/Opus 4.6/Haiku 4.5), **Google Gemini** (Gemini 2.5 Pro/Flash/Flash-Lite, 2.0 Flash), **Grok** (xAI), **OpenRouter**, **Groq**. Provider auto-detected from API key prefix. Conversational chat thread: initial generation → Review Card with [Test Preview] [Inspect in Builder] → refinement loop ("make question 3 harder"). No dropdown, no mention of "default key".
...
18. **Live integration toggles are DB-backed and OWNER-controlled.** `RuntimeConfig` (`paymentsLiveEnabled`, `bringLiveEnabled`) can be changed only via `PATCH /api/admin/runtime-config` (OWNER), and are surfaced in Admin UI at `/admin/runtime`. Runtime page includes a recent PayPal webhook feed (`/api/admin/payments/webhook-events`) backed by persisted `PaymentWebhookEvent` deliveries (event type, signature verification, processing result) for operational debugging. Production payment providers are fail-closed when required provider + webhook env vars are missing.
...
| **Pulse (Social)** | `app/feed/`, `app/pulse/` | Active | Social feed, posts, reactions, **audience-targeted visibility** (`SPECIFIC_USERS` via `visibleToUserIds`) |
| **Conversations** | `app/(protected)/` area, `lib/conversation-permissions.ts` | Active | DMs, group chats, permission checks (`canViewConversation`, `buildVisibilityWhereClause` with `SPECIFIC_USERS` support) |
| **Crypto Trading** | `components/crypto-related/` | Active | BROWSERGAME inventory, trade windows, P2P |
| **Polls** | `app/poll-test/`, `components/uicustom/polls/` | Active | 3 poll types (SURVEY, FEEDBACK, QUIZ), 11 question types, PollBuilder with 7 example templates (5 external + 2 inline), verification-weighted voting, anti-gaming, two-tier quiz feedback, **fuzzy text-answer matching** (Levenshtein), **conversational AI chat** (6-step SSE pipeline via `/api/polls/generate-stream`, refinement loop, Review Card), **interactive preview mode** (PollTakerModal with `previewData` prop), **per-question trust badges**, **6-provider BYOK** (OpenAI, Anthropic, Google Gemini, Grok, OpenRouter, Groq) with auto-detection from key prefix, **Groq free tier default** (Llama 3.3 70B) + owner-only OpenAI, **prompt injection guardrails**, **DB-backed daily quota** (5/day, survives cold starts), **scheduled daily polls** (cron + PENDING_REVIEW admin approval), **admin email alerts** (Resend) |
| **AI Credits Admin** | `app/(protected)/admin/ai-credits/`, `app/api/admin/ai-credits/route.ts` | Active | Owner/Admin visibility into AI monetization state: entitlement mode (daily cap vs credit pack), per-user usage, and remaining headroom |
