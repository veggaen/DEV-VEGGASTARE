...
export async function getPaidAiEntitlement(userId: string) {
  ...
  const granted = await dbPrisma.aiCreditEntry.aggregate({ where: { accountId, kind: { in: ["PURCHASE", "DEMO_GRANT", "PAYMENT_REVERSAL"] } }, _sum: { delta: true } });
  const totalCredits = Math.max(0, granted._sum.delta ?? 0);
