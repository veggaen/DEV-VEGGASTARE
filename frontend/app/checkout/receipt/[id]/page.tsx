...
export default async function ReceiptPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <section className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
    ...
    {!demo && <details className="mt-4 rounded-lg border border-border p-4 text-sm"><summary className="min-h-11 cursor-pointer font-medium">Original payment record</summary><p className="mt-2 text-muted-foreground">Recorded amount: {moneyString(receipt.totalOre)} NOK. Converted display values use current rates, not the historical exchange rate charged by your payment provider.</p></details>}
    ...
    <ul aria-label="Receipt items" className="mt-6 divide-y divide-border rounded-xl border border-border bg-card px-5 sm:px-6">{receipt.Order.OrderItem.map(item => <li key={item.id} className="flex min-w-0 flex-wrap justify-between gap-4 py-4"><span className="min-w-0 break-words [overflow-wrap:anywhere]">{item.title} × {item.quantity}</span><span className="max-w-full tabular-nums"><PreferredMoney amount={item.priceAtTime * item.quantity} /></span></li>)}</ul>
