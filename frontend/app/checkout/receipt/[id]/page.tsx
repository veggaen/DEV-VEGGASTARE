...
export default async function ReceiptPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <section className="mx-auto w-full max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
    ...
    <div className="mt-5"><HistoricalPriceNote /></div>
    <dl className="mt-8 grid gap-4 rounded-xl border border-border bg-card p-5 sm:grid-cols-2 sm:p-6">
      <div><dt className="text-sm text-muted-foreground">{demo ? 'Charged' : 'Order total'}</dt><dd className="mt-1 text-xl font-semibold"><PreferredMoney context="history" amount={demo ? 0 : receipt.totalOre / 100} /></dd></div>
      ...
      {receipt.refundedOre > 0 && <div><dt className="text-sm text-muted-foreground">Verified refund amount</dt><dd className="mt-1 font-semibold"><PreferredMoney context="history" amount={receipt.refundedOre / 100} /></dd></div>}
    ...
    <ul aria-label="Receipt items" className="mt-6 divide-y divide-border rounded-xl border border-border bg-card px-5 sm:px-6">{receipt.Order.OrderItem.map(item => <li key={item.id} className="flex min-w-0 flex-wrap justify-between gap-4 py-4"><span className="min-w-0 break-words [overflow-wrap:anywhere]">{item.title} × {item.quantity}</span><span className="max-w-full tabular-nums"><PreferredMoney context="history" amount={Math.round(item.priceAtTime * item.quantity * 100) / 100} /></span></li>)}</ul>
