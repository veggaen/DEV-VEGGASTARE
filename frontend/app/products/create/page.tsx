...
export default function MyProductCreationPage() {
  ...
  // Auto-close sidebar when navigating to create page
  useEffect(() => {
  ...
  return (
    <div className="h-full min-h-0 flex flex-col">
      {/* Main content: mobile scrolls (via provider), desktop fits one screen */}
      <div className="relative z-10 mx-auto w-full max-w-[1280px] px-4 py-3 sm:px-6 lg:px-8 flex-1 min-h-0 flex flex-col">
        <motion.div
          initial={reduceMotion ? undefined : { opacity: 0, y: 12 }}
          animate={reduceMotion ? undefined : { opacity: 1, y: 0 }}
          transition={{ duration: 0.35, ease: 'easeOut' }}
          className="flex-1 min-h-0 grid gap-3 grid-rows-[auto,1fr]"
        >
          {/* Header row */}
          ...
          {/* Form row - fills remaining space */}
          <motion.section
            className="relative rounded-xl border border-border bg-card shadow-sm min-h-0 h-full flex flex-col overflow-y-auto"
