...
export async function GET(request: Request) {
  try {
    ...
    let displayAccount: typeof position | null = position;
    if (demo && user?.id) {
      ...
      if (!account) displayAccount = null;
    }
    const display = displayCreditPosition(displayAccount, aiCreditEnvironment(user?.id));
