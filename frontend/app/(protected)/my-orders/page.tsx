...
import { orderMoney, orderReceiptHref, orderStatusLabel } from '@/lib/order-presentation';
          ...
          return <li key={order.id} className="min-w-0 overflow-hidden rounded-xl border border-border bg-card">
            <h2><button type="button" aria-expanded={open} aria-controls={'details-' + order.id} onClick={() => toggle(order.id)} className="flex w-full min-w-0 items-start justify-between gap-3 rounded-xl p-4 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring [@media(hover:hover)]:hover:bg-muted/40 sm:items-center sm:p-5">
              <span className="flex min-w-0 flex-1 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <span className="min-w-0 sm:text-right"><span className="block text-sm font-semibold tabular-nums">{orderMoney(demo ? 0 : order.totalAmount, order.currency)}</span><span className="mt-1 block text-xs text-muted-foreground">{demo ? 'No payment collected' : sandbox ? 'Test amount · no real money' : 'Order total'}</span></span>
            ...
            {open && <div id={'details-' + order.id} className="space-y-4 border-t border-border p-4 sm:p-5">
              ...
              {!!order.items?.length && <ul aria-label="Order items" className="divide-y divide-border">{order.items.map(item => <li key={item.id} className="flex min-w-0 items-start justify-between gap-4 py-3 text-sm"><span className="min-w-0 break-words [overflow-wrap:anywhere]">{item.title}<span className="mt-1 block text-xs text-muted-foreground">Quantity {item.quantity}</span></span><span className="shrink-0 tabular-nums">{orderMoney(item.priceAtTime * item.quantity, order.currency)}</span></li>)}</ul>}
