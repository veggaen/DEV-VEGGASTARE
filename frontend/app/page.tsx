    ...
    <div className={`relative flex flex-col items-center ${user ? 'justify-start' : 'justify-center'} h-[calc(100vh-102px)] max-h-full`}>
      ...
      {!user ? (
        <div className="relative z-10 space-y-6 p-6">
          ...
          <p>Built with Authentication service</p>
        </div>
      ) : (
        <div className="relative z-10 flex max-h-screen flex-col items-center justify-start text-black dark:text-white p-4">
          <div className="relative text-center place-items-center bg-opacity-95 dark:bg-opacity-80 bg-white dark:bg-black/50 rounded-lg p-4">
            ...
            <h2 className='font-bold font-sans text-lg xs:text-xl sm:text-2xl md:text-3xl lg:text-4xl pt-2 md:pt-4 lg:pt-6 lg:pb-6'>
              Freedom Store™
            </h2>
            <p className='py-5 hidden md:block text-xs sm:text-sm text-opacity-80 md:max-w-[800px] xl:max-w-[1000px]'>
              Welcome to our emporium, the Freedom Store. Embark on a shopping journey like no other, exploring a vast array of products that cater to every need, want, and desire. From the latest trends to timeless classics, our store is a treasure trove of possibilities, ensuring you discover the perfect items that reflect your unique style and preferences. At Where Choices Know No Limits we redefine the shopping experience, offering a world of options at your fingertips.
            </p>
