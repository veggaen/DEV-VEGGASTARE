...
export const MyProductsMap = ({ products }: MyProductsMapProps) => {
  ...
  }, [setFilteredProducts, products, setCategories]);
  ...
  const gridClasses = `sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5`;
  return (
    <div className="w-full h-full">
    {loading && <ProductsSkeleton />}
    {products && (
      <div className={`${filteredProducts.length == 1 && 'flex justify-center'}`}>
        <div className={`grid gap-3 py-6 px-2 md:px-4 ${filteredProducts.length == 1 ? 'sm:grid-cols-1 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-1' : gridClasses} `}>
          {!loading && filteredProducts.map((product, index) => (
            <div key={product.id.toString()} className={`flex flex-col ${filteredProducts.length == 1 && 'max-w-[800px] '} border border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden hover:shadow-lg transition-shadow duration-300`}>
                    ...
                    {product.image.map((image, idx) => (
                      <CarouselItem key={idx}>
                        <AspectRatio ratio={5 / 4}>
                          <Image src={image} alt={product.title} fill sizes="100%" priority className="object-fill" />
                        </AspectRatio>
                      </CarouselItem>
                    ))}
                  </CarouselContent>
