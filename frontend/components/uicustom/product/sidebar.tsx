...
interface MySidebarProductsMenuProps {
  ...
  export const MySidebarProductsMenu = ({ isOpen }: MySidebarProductsMenuProps) => {
    ...
    const [isFilterTabCategories, setIsFilterTabCategories] = useState<{ categories: boolean, price: boolean, [key: string]: boolean }>({ categories: false, price: false });
      ...
      <div className="max-w-[350px]">
        <div className={`sidebar-products rounded-md bg-white-10 ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 bg-slate-900`}>
          <div className={` overflow-y-auto specificElement182`}>
              ...
              {isFilterTabCategories.categories && (
                <div>
                  {categories && categories.map((category, index) => (
                    <div key={index} className={`bg-black/30 w-full py-1 px-2 rounded`}>
            ...
            <div className={`w-fit space-y-2 my-2`}>
              <div onClick={() => handleFilterTabOpen('price')} className="flex justify-start items-center py-1 px-2 gap-2 font-bold text-center rounded-r-xl bg-slate-800">
                <h2>Price</h2>
