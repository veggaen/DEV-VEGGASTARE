...
const MyShippingDetailsDisplay = ({ shippingResponse, toPostalCode, warehouse, userDistanceKm }: ShippingDisplayProps) => {
  ...
  // Always show local warehouse pickup if warehouse info is available
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Local Warehouse Pickup Option - shown last as alternative */}
      {showLocalPickup && warehouse && (
        <motion.div
          initial={{ opacity: 0, y: 8 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: allProducts.length * 0.05 }}
        >
          <button
