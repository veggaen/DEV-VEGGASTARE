...
export default function PricingTiers() {
  return <div>
    <div className="grid min-w-0 gap-5 md:grid-cols-3">
      {PLANS.map(plan => <article key={plan.id} className={`relative flex min-w-0 flex-col rounded-2xl border bg-card/60 p-5 sm:p-6 ${plan.featured ? "border-brand-accent/50" : "border-border"}`}>
        ...
        <Link href={plan.href} className={`mt-auto inline-flex min-h-11 items-center justify-center rounded-lg px-4 py-3 text-center text-sm font-semibold focus-visible:outline-2 focus-visible:outline-ring ${plan.featured ? "bg-brand-accent text-brand-accent-foreground hover:bg-brand-accent-hover" : "border border-border hover:bg-muted"}`}>{plan.ctaLabel}</Link>
      </article>)}
    </div>
