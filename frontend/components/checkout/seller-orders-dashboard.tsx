...
const control = 'inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border px-4 py-2 text-sm font-medium transition-colors duration-150 hover:bg-muted focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 disabled:cursor-not-allowed disabled:opacity-50';
...
export default function SellerOrdersDashboard() {
  ...
  return <div className="mx-auto w-full min-w-0 max-w-6xl space-y-6 p-4 sm:p-6 lg:p-8">
    ...
    {!parsed.success && <p role="status" className="rounded-lg border p-4 text-sm">The saved filter was invalid. Showing all orders.</p>}
...
function OrderCard({ order, expanded, onToggle }: { order: SellerOrderRow; expanded: boolean; onToggle: () => void }) {
  ...
  return <article className="min-w-0 rounded-xl border bg-card">
    <h2><button type="button" aria-expanded={expanded} aria-controls={`sale-${order.id}`} aria-label={`Order ${shortId} details`} onClick={onToggle} className="grid w-full min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-4 rounded-xl p-4 text-left transition-colors duration-150 hover:bg-muted/40 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 sm:p-5">
      <span className="grid min-w-0 gap-3 sm:grid-cols-2 sm:items-center"><span className="min-w-0"><span className="block text-sm font-semibold">#{shortId}</span><time dateTime={order.createdAt} className="mt-1 block text-xs font-normal text-muted-foreground">{date(order.createdAt)}</time></span><span className="w-fit rounded-md border px-2 py-1 text-xs font-normal">{SALE_LABELS[order.fulfilmentStatus]}</span><span className="text-base font-semibold sm:col-span-2"><PriceAmount amount={order.sellerTotal} currency={order.currency} /></span></span>
    ...
    {expanded && <div id={`sale-${order.id}`} className="space-y-6 border-t p-4 sm:p-5">
