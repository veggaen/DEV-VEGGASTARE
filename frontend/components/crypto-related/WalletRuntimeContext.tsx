...
export function useWalletsRuntime() {
  ...
  if (!ctx) throw new Error("useWalletsRuntime must be used inside WalletRuntimeProvider");
  return ctx;
}
...
export function WalletRuntimeProvider({ children }: { children: React.ReactNode }) {
  ...
  const connectEvm = useCallback(
    async (connector: Connector) => {
      ...
      try {
        ...
        if (id.includes("metamask")) setEvmBrandHint("MetaMask");
        else if (id.includes("coinbase")) setEvmBrandHint("Coinbase Wallet");
        else if (id.includes("injected")) setEvmBrandHint("Injected");
  ...
  const disconnectEvm = useCallback(async () => {
    ...
    try {
      ...
      toast.message("Disconnected EVM wallet");
    } catch (error) {
  ...
  const connectSolana = useCallback(
    async (walletName: WalletName) => {
      ...
      try {
        ...
        const label =
          solWallets.find((w) => w.adapter.name === walletName)?.adapter?.name ??
          String(walletName);
  ...
  const disconnectSolana = useCallback(async () => {
    ...
    try {
      ...
      toast.message("Disconnected Solana wallet");
    } catch (error) {
  ...
  const value: WalletContextValue = useMemo(
    () => ({
      ...
      solana: {
        ...
        brand: solBrand ?? solWallet?.adapter?.name ?? null,
      },
