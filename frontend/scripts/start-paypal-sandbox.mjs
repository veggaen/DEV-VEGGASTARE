...
const env = { ...process.env, NODE_ENV: 'production', NEXT_DIST_DIR: dist, AUTH_URL: 'http://localhost:3000',
...
delete env.VERCEL; delete env.VERCEL_ENV; delete env.PAYPAL_WEBHOOK_ID;
console.log('Sandbox credentials verified. Starting localhost:3000; real payments are disabled.');
const child = spawn(process.execPath, [require.resolve('next/dist/bin/next'), 'start', '-p', '3000'], { cwd: root, env, stdio: 'inherit' });
